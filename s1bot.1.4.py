...
TAKERS = [
     Taker("randomUser1@domain.com",[
        # Unavailable day 1 the whole day
        Unavailable(1,True,True),  
        ]),
     Taker("randomUser2@domain.com",[
        # Available on day 5th in second shift only
        Unavailable(5,True,False),  
        ]),
     Taker("randomUser3@domain.com",[
        ]),
    ]
...
def sendEvent(engEmail :str, START :str, END :str, SUBJECT :str):
    ...
    events_result = service.events().insert(
        # Get the Calendar ID from calendar.google.com, checking your calendar info in the "Integrate calendar" section
        calendarId='<truncated>@group.calendar.google.com',
        body=event).execute()
    #print(events_result)
